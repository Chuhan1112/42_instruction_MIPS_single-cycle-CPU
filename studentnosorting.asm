...
# 0155 0134
######################################################################################################################### instr.(H) ####### addr.(H)
      lui     $2, 0x0155          # high halfword the student no (last 8 digitals), use your own student no. instead!      # 0                0
      ori     $2, $2, 0x0134      # low halfword of the student no (last 8 digitals)                                       # 1                4
      sw      $2, 0($0)           # store the original stuno at data memory                                                # 2                8
      addi    $11, $0, 8          # the size of stuno, N = 8                                                               # 3                C
      lw      $1, 0x0($0)         # $1 = [0x0] = stuno                                                                     # 4                10
...
result:
      sw      $1, 0x04($0)        #  [0x04] = sortedstuno                                                                  # 1F               7C
      
      lui     $2, 0xffff          #  $1 = 0xffff0000                                                                       # 20               80
      ori     $1, $2, 0x0004      #  $1 = 0xffff0004 # switch as input                                                     # 21               84
      ori     $2, $2, 0x000c      #  $2 = 0xffff000c # seg7 as output                                                      # 22               88
...
dispsortedstuno:
      lw      $3, 0x04($0)        #  load the sorted student no.                                                           # 26               98
      j       displayseg7label                                                                                             # 27               9C
dispstuno:
      lw      $3, 0x00($0)        #  load the orginal student no.                                                          # 28               A0
displayseg7label: 
      sw      $3,  0x00($2)       #  output to seg7                                                                        # 29               A4
      j       display                                                                                                      # 2A               A8
